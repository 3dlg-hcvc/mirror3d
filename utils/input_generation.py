...
class InputGenerator(PlaneAnnotationTool):
    ...
    def __init__(self, mirror_data_main_folder, no_mirror_data_main_folder="", dataset_main_folder="",
                 json_output_folder="", split="test", anchor_normal_path="", contain_no_mirror=False,
                 split_info_folder="", max_num=0):
        """
        Args: mirror_data_main_folder : folder that contains "raw" , "instance_mask", "refined_depth" ... folders
        no_mirror_data_main_folder : folder of the original datase e.g. For Matterport3d dataset,
        no_mirror_data_main_folder should be the folder path that contains "undistorted_color_images",
        "undistorted_depth_images" folder split : "trian / test / val"
        ...
        if self.contain_no_mirror:
            assert os.path.exists(self.no_mirror_data_main_folder), \
                "please input a valid none mirror (original data) main folder"
    ...
    def generate_coco_main(self):
        ...
        for img_name in os.listdir(mirror_color_folder):
            ...
            if img_tag in mirror_split_id_list:
                mirror_color_img_list.append(os.path.join(mirror_color_folder, img_name))
        ...
        if self.contain_no_mirror:
            coco_format_save_path = os.path.join(self.json_output_folder,
                                                 "{}_{}_normal_all.json".format(self.split, normal_num))
        else:
            coco_format_save_path = os.path.join(self.json_output_folder,
                                                 "{}_{}_normal_mirror.json".format(self.split, normal_num))
        self.color_path_2_json_only_detection(mirror_color_img_list, no_mirror_color_img_list, coco_format_save_path)
    ...
    def color_path_2_json_only_detection(self, mirror_color_img_list, no_mirror_color_img_list, coco_save_path):
        """
        Args:
        ...
        for item_index, one_mirror_color_img_path in enumerate(tqdm(mirror_color_img_list)):
            ...
            if self.dataset_name == "m3d":
                mesh_raw_path_abv = rreplace(raw_img_path_abv.replace("raw", "mesh_raw_depth"), "i", "d").replace(
                    ".jpg", ".png")
                mesh_refined_path_abv = rreplace(raw_img_path_abv.replace("raw", "mesh_refined_depth"), "i",
                                                 "d").replace(".jpg", ".png")
                hole_raw_path_abv = rreplace(raw_img_path_abv.replace("raw", "hole_raw_depth"), "i", "d").replace(
                    ".jpg", ".png")
                hole_refined_path_abv = rreplace(raw_img_path_abv.replace("raw", "hole_refined_depth"), "i",
                                                 "d").replace(".jpg", ".png")
            else:
            ...
            for instance_index in np.unique(np.reshape(mask_img, (-1, 3)), axis=0):
                ...
                anchor_normal_class, anchor_normal_residual = self.get_anchor_norma_info(
        ...
        for one_no_mirror_color_img_path in tqdm(no_mirror_color_img_list):
            ...
            if self.dataset_name == "m3d":
                hole_raw_path_abv = rreplace(raw_img_path_abv.replace("color", "depth"), "i", "d").replace(".jpg",
                                                                                                           ".png")
                hole_refined_path_abv = hole_raw_path_abv
                mesh_raw_path_abv = ((rreplace(hole_raw_path_abv, "undistorted_depth_images", "mesh_images")).replace(
                    "undistorted_depth_images", "matterport_render_depth")).split(".")[0] + "_mesh_depth.png"
...
def get_k_means_normal(train_coco_json="", num_clusters=10, output_save_folder=""):
    ...
    def anchor_normal_visualization(AN_count, kmeans_normal, save_path):
    ...
    anchor_normal_visualization(AN_count, kmeans_normal, npy_save_path.replace("npy", "jpg"))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--dataset_main_folder', default="", type=str,
    ...
    generator = InputGenerator(mirror_data_main_folder=args.mirror_data_main_folder,
    ...
    if args.stage == "1":
        ...
        assert os.path.exists(
            args.split_info_folder), "please input a split information folder (please remember to down load the " \
                                     "split_info.zip from " \
                                     "http://aspis.cmpt.sfu.ca/projects/mirrors/data_release/split_info.zip) "
        if args.split == "all":
