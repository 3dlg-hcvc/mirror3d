...
from annotation.plane_annotation.plane_annotation_tool import PlaneAnnotationTool


class InputGenerator(PlaneAnnotationTool):

    def __init__(self, mask_version, mirror_data_main_folder, no_mirror_data_main_folder="", dataset_main_folder="",
                 json_output_folder="", split="test", anchor_normal_path="", contain_no_mirror=False,
                 split_info_folder="", max_num=0):
        """
        Args:
            mirror_data_main_folder : folder that contains "mirror_color_images" ,
                                      "mirror_instance_mask_{}".format(self.mask_version),
                                      "refined_depth" ... folders
            no_mirror_data_main_folder : folder of the original dataset e.g. For Matterport3D dataset,
                                         no_mirror_data_main_folder should be the folder path that contains
                                         "undistorted_color_images", "undistorted_depth_images" folder
        ...
        if self.contain_no_mirror:
            assert os.path.exists(
                self.no_mirror_data_main_folder), "please input a valid none mirror (original data) main folder"
    ...
    def generate_coco_main(self):
        ...
        mirror_color_img_list = []
        no_mirror_color_img_list = []

        # Get the mirror color image path list
        split_info_path = os.path.join(self.split_info_folder, "{}_mirror".format(self.dataset_name),
        ...
        for one_color_img_path in color_img_list:
            ...
            if train_val_test_tag in mirror_split_id_list:
                mirror_color_img_list.append(one_color_img_path)
        ...
        if self.contain_no_mirror:
            ...
            split_info_path = os.path.join(self.split_info_folder, "{}_ori".format(self.dataset_name),
                                           "{}.txt".format(self.split))
            ...
            for one_img_path in none_img_file_list:
                ...
                if self.dataset_name == "nyu":
                    ...
                    if train_val_test_tag not in mirror_split_id_list and train_val_test_tag in no_mirror_split_id_list:
                        no_mirror_color_img_list.append(one_img_path)
                else:
        ...
        if self.contain_no_mirror:
            coco_format_save_path = os.path.join(self.json_output_folder,
                                                 "{}_{}_{}_normal_all.json".format(self.split, normal_num,
                                                                                   self.mask_version))
        else:
            coco_format_save_path = os.path.join(self.json_output_folder,
                                                 "{}_{}_{}_normal_mirror.json".format(self.split, normal_num,
                                                                                      self.mask_version))
        self.colorPath_2_json_only_detection(mirror_color_img_list, no_mirror_color_img_list, coco_format_save_path)
    ...
    def get_anchor_normal_info(self, mirror_normal):
        ...
        closest_distance = 100  # init to be a large number
    ...
    def colorPath_2_json_only_detection(self, mirror_color_img_list, no_mirror_color_img_list, coco_save_path):
        ...
        # Get COCO annotation for images contain mirror
        for item_index, one_mirror_color_img_path in enumerate(tqdm(mirror_color_img_list)):
            ...
            mirror_instance_mask_path = one_mirror_color_img_path.replace("mirror_color_images",
                                                                          "mirror_instance_mask_{}".format(
                                                                              self.mask_version)).replace(".jpg",
                                                                                                          ".png")
            img_info_path = one_mirror_color_img_path.replace("mirror_color_images", "mirror_plane").split(".")[
                                0] + ".json"
            ...
            if self.dataset_name == "mp3d":
                raw_mesh_d_path_abv = rreplace(raw_img_path_abv.replace("mirror_color_images", "raw_meshD"), "i",
                                               "d").replace(".jpg", ".png")
                refined_mesh_d_path_abv = rreplace(
                    raw_img_path_abv.replace("mirror_color_images", "refined_meshD_{}".format(self.mask_version)), "i",
                    "d").replace(".jpg", ".png")
                raw_sensor_d_path_abv = rreplace(raw_img_path_abv.replace("mirror_color_images", "raw_sensorD"), "i",
                                                 "d").replace(".jpg", ".png")
                refined_sensor_d_path_abv = rreplace(
                    raw_img_path_abv.replace("mirror_color_images", "refined_sensorD_{}".format(self.mask_version)),
                    "i", "d").replace(".jpg", ".png")
            else:
                raw_sensor_d_path_abv = raw_img_path_abv.replace("mirror_color_images", "raw_sensorD").replace(".jpg",
                                                                                                               ".png")
                refined_sensor_d_path_abv = raw_img_path_abv.replace("mirror_color_images", "refined_sensorD_{}".format(
                    self.mask_version)).replace(".jpg", ".png")
                raw_mesh_d_path_abv = raw_sensor_d_path_abv
                refined_mesh_d_path_abv = refined_sensor_d_path_abv
            ...
            image = {
                ...
                "raw_meshD_path": raw_mesh_d_path_abv,
            ...
            for instance_index in np.unique(mask_img):
                ...
                annotation["refined_meshD_path"] = refined_mesh_d_path_abv
                annotation["refined_sensorD_path"] = refined_sensor_d_path_abv
        ...
        # Get COCO annotation for images don't have mirror
        ...
        for one_no_mirror_color_img_path in tqdm(no_mirror_color_img_list):
            ...
            if self.dataset_name == "mp3d":
                raw_sensor_d_path_abv = rreplace(raw_img_path_abv.replace("color", "depth"), "i", "d").replace(".jpg",
                                                                                                               ".png")
                refined_sensor_d_path_abv = raw_sensor_d_path_abv
                raw_mesh_d_path_abv = ((rreplace(raw_sensor_d_path_abv, "undistorted_depth_images",
                                                 "mesh_images")).replace("undistorted_depth_images",
                                                                         "matterport_render_depth")
                                       ).split(".")[0] + "_mesh_depth.png"
                refined_mesh_d_path_abv = raw_mesh_d_path_abv
            else:
                raw_sensor_d_path_abv = raw_img_path_abv.replace("color", "depth").replace(".jpg", ".png")
                refined_sensor_d_path_abv = raw_sensor_d_path_abv
                raw_mesh_d_path_abv = raw_sensor_d_path_abv
                refined_mesh_d_path_abv = raw_sensor_d_path_abv
...
def get_k_means_normal(train_coco_json="", num_clusters=10, output_save_folder=""):
    from collections import Counter
    from kmeans_pytorch import kmeans

    def anchor_normal_visualization(an_count, k_means_normal, save_path):

        color_list = ["orange", "purple", "pink", "yellow", "cyan", "gray", "lime", "tan", "gold", "salmon", "plum",
                      "peru", "teal", "olive"]
        # plt.cla()
        fig = plt.figure()
        title = ""
        ...
        for anchor_id, one_normal in enumerate(k_means_normal):
            # (X, Z, Y)
            ax.quiver(0, 0, 0, one_normal[0] * 1, one_normal[1] * 1, one_normal[2] * 1, length=2000,
                      color=color_list[anchor_id], arrow_length_ratio=0.05)  # X
            ax.text(one_normal[0] * 2000, one_normal[1] * 2000, one_normal[2] * 2000,
                    "{}:{}".format(anchor_id, an_count[anchor_id]), color=color_list[anchor_id], fontsize=8)

        ax.view_init(-80, -90)
        # plt.show()
        plt.savefig(save_path)
        print("normal visualization saved to : ", save_path)
    ...
    k_means_normal = cluster_centers.numpy()
    ...
    np.save(npy_save_path, k_means_normal)
    print("numpy saved to : ", npy_save_path)
    an_count = Counter(cluster_ids_x.tolist())
    anchor_normal_visualization(an_count, k_means_normal, npy_save_path.replace("npy", "jpg"))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--no_mirror_data_main_folder', default="", type=str, help="dataset main folder")
    parser.add_argument('--dataset_main_folder', default="", type=str,
                        help="output path in .json will be the relative path to --dataset_main_folder")
    parser.add_argument('--split_info_folder', default="", type=str, help="split_info.zip unzip folder")
    parser.add_argument('--json_output_folder', default="", type=str, help="dataset main folder")
    parser.add_argument('--split', default="all", type=str, help="train / test/ val")
    parser.add_argument('--contain_no_mirror', help='do multi-process or not', action='store_true')
    parser.add_argument('--anchor_normal_path', default="", type=str, help="anchor normal path")
    parser.add_argument('--max_num', default=0, type=int, help="max number of none-mirror samples")
    # Args for --stage 2
    parser.add_argument('--output_save_folder', default="", type=str, help="k_means anchor normal saved path")
    parser.add_argument(
        '--coco_json', default="", type=str, help="coco format json file to generate the k_means normal")
    parser.add_argument('--num_clusters', default=10, type=int, help="number of cluster center")
    parser.add_argument('--mask_version', default="precise", help="2 mask version : precise/ coarse")
    args = parser.parse_args()
    generator = InputGenerator(mask_version=args.mask_version, mirror_data_main_folder=args.mirror_data_main_folder,
    ...
    if args.stage == "1":
        ...
        assert os.path.exists(args.split_info_folder), "please input a split information folder (please remember to " \
                                                       "down load the split_info.zip from " \
                                                       "https://aspis.cmpt.sfu.ca/projects/mirrors/data_release" \
                                                       "/split_info.zip) "
        if args.split == "all":
        ...
        get_k_means_normal(train_coco_json=args.coco_json, num_clusters=args.num_clusters,
