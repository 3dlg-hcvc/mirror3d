...
def get_filtered_percentage(dataset="m3d"):
    from tqdm import tqdm
    data_main_path = "/project/3dlg-hcvc/mirrors/www/Mirror3D_final/{}".format(dataset)
    test_json = "/project/3dlg-hcvc/mirrors/www/Mirror3D_final/{}/with_mirror/precise/network_input_json/test_10_normal_mirror.json".format(
        dataset)
    ...
    for item in tqdm(test_info["images"]):
        ...
        instance_mask = cv2.imread(os.path.join(data_main_path, item["img_path"].replace("raw", "instance_mask")),
        ...
        # if ((raw_depth < 1e-3)*instance_mask).sum() / instance_mask.sum() > 0.5: sample_name = item[
        # "mesh_raw_path"].split("/")[-1] colored_depth_path =
        # "/project/3dlg-hcvc/mirrors/www/cr_vis/scannet_result_vis/sensor-D/colored_pred_depth/{}".format(
        # sample_name) color_img_path = os.path.join(data_main_path, item["img_path"]) print(colored_depth_path,
        # color_img_path)
