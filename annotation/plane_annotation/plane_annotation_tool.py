...
class PlaneAnnotationTool:
    """
    The PlaneAnnotationTool class currently does the following:
    1. anno_env_setup          
    2. anno_plane_update_img_info
    3. anno_update_depth_from_img_info
    """

    def __init__(self, data_main_folder=None, process_index=0, multi_processing=False, border_width=50, f=519,
                 anno_output_folder=None):
        """
        Initialization
        ...
        self.check_file()
        self.color_img_list = [os.path.join(data_main_folder, "raw", i) for i in
                               os.listdir(os.path.join(data_main_folder, "raw"))]
        self.color_img_list.sort()
        if multi_processing:
            self.color_img_list = self.color_img_list[process_index:process_index + 1]
        # Because we use "cv2.getStructuringElement(cv2.MORPH_ELLIPSE ***" and the anchor of the element is at the
        # center, so actual width is half of self.border_width
        self.border_width = border_width * 2
        self.f = f
        if anno_output_folder is None or not os.path.exists(anno_output_folder):
            self.anno_output_folder = data_main_folder
            print("########## NOTE output saved to {}, this may overwrite your current information ############".format(
                self.anno_output_folder))
        else:
    ...
    def check_file(self):
        ...
        for file_name in os.listdir(raw_foler):
            ...
            if os.path.exists(mask_file_path) and os.path.exists(raw_file_path) and os.path.exists(
                    depth_file_path) and os.path.exists(img_info_file_path):
                continue
            else:
                data_correct = False
                self.save_error_raw_name(file_name)
                print(" path not exisits : {} {} mask {} {}raw {} depth {} {} img_info {} {}".format(file_name,
                                                                                                     os.path.exists(
                                                                                                         mask_file_path),
                                                                                                     mask_file_path,
                                                                                                     os.path.exists(
    ...
    def anno_env_setup(self):
        ...
        for color_img_path in self.color_img_list:
            # Get paths
            sample_name = os.path.split(color_img_path)[1].split(".")[0]
            ...
            for instance_index in np.unique(np.reshape(mask, (-1, 3)), axis=0):
                ...
                instance_tag = sample_name + instance_tag
                ...
                mirror_border_mask = cv2.dilate(binary_instance_mask, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (
                    self.border_width, self.border_width))) - binary_instance_mask
                ...
                pcd, plane_parameter = refine_pcd_by_mirror_border(binary_instance_mask, mirror_border_mask,
                                                                   depth_img_path, color_img_path, self.f)
                one_plane_para_save_path = os.path.join(plane_parameter_save_folder, "{}.json".format(sample_name))
    ...
    def anno_plane_update_img_info(self):
        """
        ...
        annotation_progress_save_folder = os.path.join(self.anno_output_folder, "anno_progress")
        ...
        while 1:
            ...
            current_pcd_id = current_pcd_path.split("/")[-1].split("_idx_")[0]

            # If one instance in the sample is negative; then this sample is invalid
            if current_pcd_id in self.error_id:
            ...
            mirror_border_vis_save_folder = os.path.join(self.anno_output_folder, "border_vis")
            masked_image_path = os.path.join(mirror_border_vis_save_folder,
                                             "{}.jpg".format(current_pcd_path.split("/")[-1].split(".")[0]))
            ...
            if self.is_matterport3d:
                depth_img_path = os.path.join(self.data_main_folder, "mesh_raw_depth",
            ...
            plane_parameter = read_json(os.path.join(self.anno_output_folder, "img_info", "{}.json".format(img_name)))[
                pcd_name.split("_idx_")[1]]["plane_parameter"]
            ...
            option_list.add_option("back n",
                                   "BACK : return n times (e.g. back 3 : give up the recent 3 annotated sample and go back)")
            ...
            if input_option == "t":
                ...
                try:
                    ...
                    manual_percentage = (manual_adjust_num / (self.sample_index - annotation_start_index)) * 100
                    print(
                        "Reference annotation speed {:.2f} s/sample; Estimate remaining time {:.1f} h; manual adjust "
                        "{:.2f}%".format(
                            refer_speed, left_h, manual_percentage))
                except:
                    pass
                exit(1)
                ...
                plane_parameter = get_parameter_from_plane_adjustment(pcd,
                                                                      get_mirror_init_plane_from_3points(p1, p2, p3),
                                                                      init_step_size)

                one_plane_para_save_path = os.path.join(os.path.join(self.anno_output_folder, "img_info"),
                                                        "{}.json".format(img_name))
                ...
                instance_mask = get_grayscale_instanceMask(cv2.imread(mask_path), instance_id)
                mirror_pcd = get_mirrorPoint_based_on_plane_parameter(f=self.f, plane_parameter=plane_parameter,
                                                                      mirror_mask=instance_mask,
                                                                      color_img_path=color_img_path, color=[1, 1, 0])
                init_step_size = ((np.max(np.array(pcd.points)[:, 0])) - (np.min(np.array(pcd.points)[:, 0]))) / 300
                while 1:
                    min_adjust_option_list = ToolOption()
                    min_adjust_option_list.add_option("f",
                                                      "FINISH : update hole_refined_depth/ mesh_refined_depth/ "
                                                      "img_info and EXIT")
                    min_adjust_option_list.add_option("a",
                                                      "ADJUST : adjust the plane parameter based on current plane "
                                                      "parameter")
                    ...
                    if min_input_option == "f":
                        ...
                        [p1, p2, p3] = get_picked_points(pcd)
                        plane_parameter = get_parameter_from_plane_adjustment(pcd,
                                                                              get_mirror_init_plane_from_3points(p1, p2,
                                                                                                                 p3),
                                                                              init_step_size)
                        mirror_pcd = get_mirrorPoint_based_on_plane_parameter(f=self.f, plane_parameter=plane_parameter,
                                                                              mirror_mask=instance_mask,
    ...
    def anno_move_only_mask(self):
        ...
        for color_img_path in self.color_img_list:
            ...
            if sample_name in self.error_id:
                command = "find {} -type f | grep {}".format(self.data_main_folder, sample_name)
                ...
                if self.is_matterport3d:
                    sample_name = rreplace(sample_name, "i", "d")
                    command = "find {} -type f | grep {}".format(self.data_main_folder, sample_name)
                    for src_path in os.popen(command).readlines():
                        src_path = src_path.strip()
                        dst_path = src_path.replace("with_mirror", "only_mask")
    ...
    def anno_update_depth_from_img_info(self):
        ...
        for color_img_path in self.color_img_list:
            sample_name = os.path.split(color_img_path)[1].split(".")[0]
            mask_img_path = os.path.join(self.data_main_folder, "instance_mask", "{}.png".format(sample_name))
            mask = cv2.imread(mask_img_path)

            one_info_file_path = os.path.join(img_info_save_folder, "{}.json".format(sample_name))
            ...
            for one_info in info.items():
                ...
                if self.is_matterport3d:
                    depth_file_name = "{}.png".format(rreplace(sample_name, "i", "d"))
                    ...
                    cv2.imwrite(mesh_refined_depth_path,
                                refine_depth_with_plane_parameter_mask(plane_parameter, binary_instance_mask,
                                                                       cv2.imread(mesh_raw_depth_path,
                                                                                  cv2.IMREAD_ANYDEPTH), self.f))
                    print("update depth {}".format(mesh_refined_depth_path))
                else:
                    depth_file_name = "{}.png".format(sample_name)
                ...
                cv2.imwrite(hole_refined_depth_path,
                            refine_depth_with_plane_parameter_mask(plane_parameter, binary_instance_mask,
                                                                   cv2.imread(hole_raw_depth_path, cv2.IMREAD_ANYDEPTH),
                                                                   self.f))
    ...
    def get_progress(self):
        ...
        error_txt = os.path.join(annotation_progress_save_folder, "error_list.txt")
        correct_txt = os.path.join(annotation_progress_save_folder, "correct_list.txt")
    ...
    def adjust_one_sample_plane(self, instance_index=None, img_name=None):
        ...
        import open3d as o3d
        if self.is_matterport3d:
            hole_raw_depth_path = os.path.join(self.data_main_folder, "hole_raw_depth",
                                               "{}.png".format(rreplace(img_name, "i", "d")))
            mesh_raw_depth_path = os.path.join(self.data_main_folder, "mesh_raw_depth",
                                               "{}.png".format(rreplace(img_name, "i", "d")))
            mesh_refined_depth_path = os.path.join(self.anno_output_folder, "mesh_refined_depth",
                                                   "{}.png".format(rreplace(img_name, "i", "d")))
            hole_refined_depth_path = os.path.join(self.anno_output_folder, "hole_refined_depth",
                                                   "{}.png".format(rreplace(img_name, "i", "d")))
        else:
        ...
        mirror_pcd = get_mirrorPoint_based_on_plane_parameter(f=self.f, plane_parameter=plane_parameter,
                                                              mirror_mask=instance_mask, color_img_path=color_img_path,
                                                              color=[0, 0, 1])
        o3d.visualization.draw_geometries([pcd, mirror_pcd])
        init_step_size = ((np.max(np.array(pcd.points)[:, 0])) - (np.min(np.array(pcd.points)[:, 0]))) / 300
        ...
        while 1:
            ...
            if input_option == "f":
                save_plane_parameter_2_json(plane_parameter, one_plane_para_save_path, instance_id)
                cv2.imwrite(hole_refined_depth_path,
                            refine_depth_with_plane_parameter_mask(plane_parameter, instance_mask,
                                                                   cv2.imread(hole_raw_depth_path, cv2.IMREAD_ANYDEPTH),
                                                                   self.f))
                if self.is_matterport3d:
                    cv2.imwrite(mesh_refined_depth_path,
                ...
                mirror_pcd = get_mirrorPoint_based_on_plane_parameter(f=self.f, plane_parameter=plane_parameter,
                                                                      mirror_mask=instance_mask,
                                                                      color_img_path=color_img_path, color=[0, 0, 1])
                o3d.visualization.draw_geometries([pcd, mirror_pcd])
    ...
    def manual_clamp_one_sample(self, instance_index=None, img_name=None):
        ...
        while 1:
            ...
            option_list.add_option("d",
                                   "DISTANCE : the clamping distance_threshold; distance over distance_threshold will "
                                   "be clamped")
            ...
            if input_option == "d":
                clamp_dis = int(input("please input new clamping distance (default : 100)"))
            elif input_option == "f":
                ...
                clamp_mask = get_triangle_mask(three_points_2D, w, h)
                refined_depth_to_clamp = clamp_pcd_by_mask(depth_to_refine=refined_depth_to_clamp, f=self.f,
                                                           clamp_mask=clamp_mask, plane_parameter=plane_parameter,
                                                           clamp_dis=clamp_dis)
...
class DataPostProcessing(PlaneAnnotationTool):

    def __init__(self, data_main_folder=None, process_index=0, multi_processing=False, border_width=50, f=519,
                 anno_output_folder=None, expand_range=100, clamp_dis=100):
        """
        Initilization
        ...
        for color_img_path in self.color_img_list:
            ...
            for one_info in info.items():
                ...
                if self.is_matterport3d:
                    ...
                    depth_img_path = os.path.join(self.data_main_folder, "hole_raw_depth",
                                                  "{}.png".format(rreplace(img_name, "i", "d")))
                else:
                    depth_img_path = os.path.join(self.data_main_folder, "hole_raw_depth", "{}.png".format(img_name))
                ...
                mirror_border_mask = cv2.dilate(instance_mask, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (
                    self.border_width, self.border_width))) - cv2.erode(instance_mask,
                ...
                if self.is_matterport3d:
                    ...
                    mesh_refined_depth_path = os.path.join(self.data_main_folder, "mesh_refined_depth", depth_file_name)
                    cv2.imwrite(mesh_refined_depth_path,
                                clamp_pcd_by_bbox(mirror_bbox=mirror_bbox, depth_img_path=mesh_refined_depth_path,
                                                  f=self.f, mirror_border_mask=mirror_border_mask,
                                                  plane_parameter=one_info[1]["plane_parameter"],
                                                  expand_range=self.expand_range, clamp_dis=self.clamp_dis))
                    print("update depth {}".format(mesh_refined_depth_path))

                # Refine hole raw depth
                hole_refined_depth_path = os.path.join(self.data_main_folder, "hole_refined_depth", depth_file_name)
                cv2.imwrite(hole_refined_depth_path,
                            clamp_pcd_by_bbox(mirror_bbox=mirror_bbox, depth_img_path=hole_refined_depth_path, f=self.f,
                                              mirror_border_mask=mirror_border_mask,
                                              plane_parameter=one_info[1]["plane_parameter"],
                                              expand_range=self.expand_range, clamp_dis=self.clamp_dis))
                print("update depth {}".format(hole_refined_depth_path))

    def update_img_info_based_on_depth(self):
        """
        Update img_info based on refined depth
        ...
        for color_img_path in self.color_img_list:
            img_name = os.path.split(color_img_path)[1].split(".")[0]
            if self.is_matterport3d:
                depth_img_path = os.path.join(self.data_main_folder, "mesh_refined_depth",
                                              "{}.png".format(rreplace(img_name, "i", "d")))
            else:
            ...
            for instance_index in np.unique(np.reshape(mask, (-1, 3)), axis=0):
                ...
                plane_parameter = get_mirror_parameter_from_xyzs_by_ransac(
                    get_points_in_mask(self.f, depth_img_path, color_img_path, mirror_mask=binary_instance_mask))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--specific_clamp_parameter_json', default="",
        help="json file that contain the sample id and relevant expand_range; only useful for multi-processing")
    parser.add_argument(
        '--instance_index', default="")
    args = parser.parse_args()

    if args.stage == "1":
        plane_anno_tool = PlaneAnnotationTool(data_main_folder=args.data_main_folder,
                                              process_index=args.process_index,
                                              multi_processing=args.multi_processing, border_width=args.border_width,
                                              f=args.f, anno_output_folder=args.anno_output_folder)
        ...
        plane_anno_tool.anno_plane_update_img_info()
    elif args.stage == "3":
        plane_anno_tool = PlaneAnnotationTool(data_main_folder=args.data_main_folder,
                                              process_index=args.process_index,
                                              multi_processing=args.multi_processing, border_width=args.border_width,
                                              f=args.f, anno_output_folder=args.anno_output_folder)
        plane_anno_tool.anno_update_depth_from_img_info()
    elif args.stage == "4":
        plane_anno_tool = DataPostProcessing(data_main_folder=args.data_main_folder, process_index=args.process_index,
                                             multi_processing=args.multi_processing, border_width=args.border_width,
                                             f=args.f, anno_output_folder=args.anno_output_folder,
                                             expand_range=args.expand_range, clamp_dis=args.clamp_dis)
        ...
        assert os.path.exists(
            args.color_img_list), "please input a valid --color_img_list before moving data based on annotation result"
        ...
        plane_anno_tool = DataPostProcessing(data_main_folder=args.data_main_folder, process_index=args.process_index,
                                             multi_processing=args.multi_processing, border_width=args.border_width,
                                             f=args.f, anno_output_folder=args.anno_output_folder)
        plane_anno_tool.update_img_info_based_on_depth()
    elif args.stage == "6":
        plane_anno_tool = PlaneAnnotationTool(data_main_folder=args.data_main_folder,
                                              process_index=args.process_index,
                                              multi_processing=args.multi_processing, border_width=args.border_width,
