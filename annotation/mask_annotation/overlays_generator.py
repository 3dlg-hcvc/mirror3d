...
def generate_overlay_masks_and_check(args_obj):
    raw_filenames = os.listdir(args_obj.raw_folder)
    ...
    if args_obj.multi_processing:
        raw_filenames.sort()
        raw_filenames = raw_filenames[args_obj.process_index:args_obj.process_index+1]
    # Check consistency
    len_raw = len(raw_filenames)
    ...
    for file in tqdm(raw_filenames, desc='Processing'):
        path = os.path.join(args_obj.output, file[:-4])

        raw = Image.open(os.path.join(args_obj.raw, file))
    ...
    if len(label_error_list) != 0:
        log_path = os.path.abspath(os.path.join(args_obj.output, "label_error_ids.txt"))
        print("\nWarning: " + str(len(label_error_list)) + " masks have inconsistent labels, ids store at " + log_path + "\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--process_index', help="process index", default=0, type=int)
    parser.add_argument('--multi_processing', help='do multi-process or not', action='store_true')
    args = parser.parse_args()
    generate_overlay_masks_and_check(args)
