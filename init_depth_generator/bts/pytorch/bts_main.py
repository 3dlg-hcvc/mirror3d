...
parser.add_argument('--model_name',                type=str,   help='model name', default='nyu_refineD')
#TODO
...
# TODO
parser.add_argument('--coco_val',                  type=str,   help='coco json path', default='/project/3dlg-hcvc/jiaqit/m3d/m3d_unzip/coco_input/DE_1280_1024_all_1000debug/pos_test_only_DE.json')
# TODO
parser.add_argument('--coco_train',                type=str,   help='coco json path', default='/project/3dlg-hcvc/jiaqit/m3d/m3d_unzip/coco_input/DE_1280_1024_all_1000debug/pos_train_only_DE.json')
# TODO
...
parser.add_argument('--coco_root',           type=str,   help='coco data root', 
    default="/project/3dlg-hcvc/jiaqit/m3d/m3d_unzip")
parser.add_argument('--coco_root',             type=str,   help='coco data root', 
    default="/project/3dlg-hcvc/jiaqit/m3d/m3d_unzip")
# TODO
parser.add_argument('--coco_focal_len',            type=str,   help='focal length of input data; correspond to INPUT DEPTH!', default="546")
# TODO 
...
parser.add_argument('--checkpoint_save_freq',                 type=int,   help='Checkpoint saving frequency in global steps', default=500)
...
# Online eval
parser.add_argument('--do_online_eval',            type=bool,  help='if set, perform online eval in every eval_freq steps', default=True)
...
# train_setting = "arguments_train_nyu.txt"
# arg_filename_with_prefix = '@' + train_setting #sys.argv[1]
# args = parser.parse_args([arg_filename_with_prefix])
args = parser.parse_args()
args.coco_input = True # TODO delete later
print("coco_input : ", args.coco_input)
# else:
#     args = parser.parse_args()

# if args.mode == 'train' and not args.resume_checkpoint_path:
#     from bts import *

# elif args.mode == 'train' and args.resume_checkpoint_path:
#     model_dir = os.path.dirname(args.resume_checkpoint_path)
#     model_name = os.path.basename(model_dir)
#     model_name = os.path.split(args.resume_checkpoint_path)
#     import sys
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                    ...
                    #     train_writer.add_image('depth_gt/image/{}'.format(i), normalize_result(1/depth_gt[i, :, :, :].data), global_step)
                    #     train_writer.add_image('depth_est/image/{}'.format(i), normalize_result(1/depth_est[i, :, :, :].data), global_step)
                    #     train_writer.add_image('reduc1x1/image/{}'.format(i), normalize_result(1/reduc1x1[i, :, :, :].data), global_step)
                    #     train_writer.add_image('lpg2x2/image/{}'.format(i), normalize_result(1/lpg2x2[i, :, :, :].data), global_step)
                    #     train_writer.add_image('lpg4x4/image/{}'.format(i), normalize_result(1/lpg4x4[i, :, :, :].data), global_step)
                    #     train_writer.add_image('lpg8x8/image/{}'.format(i), normalize_result(1/lpg8x8[i, :, :, :].data), global_step)
                    #     train_writer.add_image('image/image/{}'.format(i), inv_normalize(image[i, :, :, :]).data, global_step)
                    # train_writer.flush()
                    
            if  global_step and global_step % args.checkpoint_save_freq == 0:
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                    ...
                    torch.save(checkpoint, os.path.join(args.log_directory , model_save_name))
            ...
            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                ...
                if eval_measures is not None:
                    for i in range(9):
                        train_writer.add_scalar(eval_metrics[i], eval_measures[i].cpu(), int(global_step))
                        measure = eval_measures[i]
                        is_best = False

                        if i < 6:
                            temp_lower[i] =  measure.item()
                        if i >= 6:
                            temp_higher[i-6] =  measure.item()
                        ...
                        if is_best:
                            ...
                            # model_path = os.path.join(args.log_directory , old_best_name)
                        ...
                        # print("debugging !!!!!! :", i, measure, is_best, temp_lower, temp_higher)
                        if eval_metrics[i] == "rms":
                            checkpoint = {'global_step': global_step,
                                            'model': model.state_dict(),
                                            'optimizer': optimizer.state_dict(),
                                            'best_eval_measures_higher_better': temp_higher,
                                            'best_eval_measures_lower_better': temp_lower,
                                            'best_eval_steps': global_step
                                            }
                            model_save_name = 'model-{}-checkpoint_rms_{}_{:.5f}'.format(global_step, eval_metrics[i], measure)
                            torch.save(checkpoint, os.path.join(args.log_directory , model_save_name) )
                            print('New best for {}. Saving model: {}'.format(eval_metrics[i], model_save_name))
                    # train_writer.flush()
                model.train()
...
def main():
    ...
    args.log_directory = os.path.join("output", "bts_{}_{}".format(args.model_name, time_tag))
    os.makedirs(args.log_directory, exist_ok=True)
    config_save_path = os.path.join(args.log_directory, "setting.txt")
    # TODO debug and see how to save args
    ...
    # copy setting 
    # args_out_path = args.log_directory + '/' + args.model_name + '/' + train_setting
    # command = 'cp ' + train_setting + ' ' + args_out_path
    # os.system(command)
