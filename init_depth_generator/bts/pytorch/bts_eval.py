...
def test(params):
    ...
    if os.path.isdir(args.resume_checkpoint_path):
        import glob
        models = [f for f in glob.glob(args.resume_checkpoint_path + "/model*")]
        ...
        steps.add('{:06d}'.format(int(args.resume_checkpoint_path.split('-')[-1])))
    ...
    for step in steps:
        if os.path.isdir(args.resume_checkpoint_path):
            checkpoint = torch.load(os.path.join(args.resume_checkpoint_path, 'model-' + str(int(step))))
