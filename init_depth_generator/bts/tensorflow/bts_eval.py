...
parser.add_argument('--resume_checkpoint_path',     type=str,   help='path to a specific checkpoint to load', default='')
...
def test(params):
    ...
    if os.path.exists(args.resume_checkpoint_path + '.meta'):
        steps = [str(args.resume_checkpoint_path).split('/')[-1].split('-')[-1]]
    else:
        with open(args.resume_checkpoint_path + '/checkpoint') as file:
        ...
        lines = []
        if os.path.exists(args.resume_checkpoint_path + '/evaluated_checkpoints'):
            with open(args.resume_checkpoint_path + '/evaluated_checkpoints') as file:
        ...
        if args.output_directory != '':
            ...
            summary_path = os.path.join(args.resume_checkpoint_path, 'eval')
        write_summary = True
    ...
    time_modified = os.path.getmtime(args.resume_checkpoint_path + 'checkpoint')
    ...
    with tf.device('/cpu:0'):
        for step in steps:
            ...
            if os.path.exists(args.resume_checkpoint_path + '.meta'):
                ...
                restore_path = os.path.join(args.resume_checkpoint_path, 'model-' + str(int(step)))
            ...
            if write_summary:
                ...
                with open(os.path.dirname(args.resume_checkpoint_path) + '/evaluated_checkpoints', 'a') as file:
                        file.write(step + '\n')
