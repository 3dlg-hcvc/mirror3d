...
if __name__=='__main__':
    ...
    if train_args.refined_depth:
        ...
        tag = "raw_depth"
    train_args.results_dir = os.path.join(train_args.results_dir, tag + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()))
    os.makedirs(train_args.results_dir, exist_ok=True)
    config_save_path = os.path.join(train_args.results_dir, "setting.txt")
    with open(config_save_path, "w") as file:
        ...
        for item in train_args.__dict__.items():
            ...
            file.write("\n")
    print("output saved to : ", train_args.results_dir)
    ...
    # temp = train_args.coco_train
    # train_args.coco_train = train_args.coco_val
    val_dataloader = CustomerDataLoader(val_args)
    val_datasize = len(val_dataloader)
    # train_args.coco_train = temp
