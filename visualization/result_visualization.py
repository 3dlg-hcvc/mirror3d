...
from visualization.dataset_visualization import DatasetVisualization
from tqdm import tqdm


class DatasetVisualization(DatasetVisualization):

    def __init__(self, pred_w=480, pred_h=640, dataset_main_folder=None, test_json="", method_tag="mirror3D",
                 process_index=0, multi_processing=False,
    ...
    def generate_color_depth_for_one_pred(self, color_img_path):
        ...
        for item in self.method_predFolder.items():
            ...
            if self.is_matterport3d:
                ...
                colored_pred_depth_save_path = os.path.join(one_colored_pred_depth_folder,
                                                            rreplace(sample_name, "i", "d"))
                colored_pred_error_map_save_path = os.path.join(one_colored_pred_error_map_folder,
                                                                rreplace(sample_name, "i", "d"))
            ...
            if self.is_matterport3d:
                ...
                depth_shift = 1000
            gt_depth = np.asarray(
                cv2.resize(gt_depth, dsize=(self.pred_w, self.pred_h), interpolation=cv2.INTER_NEAREST),
                dtype=np.float32) / depth_shift
            pred_depth = np.asarray(
                cv2.resize(pred_depth, dsize=(self.pred_w, self.pred_h), interpolation=cv2.INTER_NEAREST),
                dtype=np.float32) / depth_shift
    ...
    def generate_pcd_for_one_prediction(self, color_img_path):
        ...
        for item in self.method_predFolder.items():
            ...
            pcd = get_pcd_from_rgbd_depthPath(f=self.f, depth_img_path=pred_depth_img_path,
                                              color_img_path=color_img_path, w=self.pred_w, h=self.pred_h)
    ...
    def generate_html(self, vis_saved_main_folder="", sample_num_per_page=50, template_path=""):
        """
        (1) under vis_saved_folder there should only be the visualization output sub-folders and nothing else
        ...
        """
        if self.is_matterport3d:
            method_folder_list = ["VNL+Mirror3DNet", "VNL-raw", "BTS+Mirror3DNet", "BTS-raw", "VNL-ref", "BTS-ref",
                                  "Mirror3DNet-raw", "Mirror3DNet-DE-raw", "PlaneRCNN-raw", "PlaneRCNN-DE-raw",
                                  "saic+Mirror3DNet", "saic-raw", "Mirror3DNet-ref", "Mirror3DNet-DE-ref",
                                  "PlaneRCNN-ref", "PlaneRCNN-DE-ref", "saic-ref", "mesh-D+Mirror3DNet",
                                  "sensor-D+Mirror3DNet", "mesh-D", "sensor-D"]
            # method_folder_list = ["GT", "meshD", "meshD_Mirror3DNet", "BTS_refD", "BTS_rawD", "BTS_Mirror3DNet",
            # "VNL_refD", "VNL_rawD", "VNL_Mirror3DNet", "SAIC_refD", "SAIC_rawD", "SAIC_Mirror3DNet"]
        else:
            method_folder_list = ["sensor-D+Mirror3DNet-m3d", "saic+Mirror3DNet-m3d", "VNL+Mirror3DNet-m3d",
                                  "BTS+Mirror3DNet-m3d", "saic-ref", "saic-raw", "VNL-ref", "VNL-raw", "BTS-ref",
                                  "BTS-raw", "sensor-D", "GT"]
        # method_folder_list = ["GT", "sensorD", "sensorD_Mirror3DNet", "BTS_refD", "BTS_rawD", "BTS_Mirror3DNet",
        # "VNL_refD", "VNL_rawD", "VNL_Mirror3DNet", "SAIC_refD", "SAIC_rawD", "SAIC_Mirror3DNet"]
        colorImgSubset_list = [self.color_img_list[x:x + sample_num_per_page] for x in
                               range(0, len(self.color_img_list), sample_num_per_page)]
        ...
        for html_index, one_colorSubset in enumerate(colorImgSubset_list):
            ...
            for one_color_img_path in self.color_img_list:
                ...
                for one_method_name in method_folder_list:
                    ...
                    one_front_view_img = os.path.join(one_method_folder_path, "pred_depth_ply",
                                                      "front_{}".format(sample_name))
                    one_topdown_view_img = os.path.join(one_method_folder_path, "pred_depth_ply",
                                                        "topdown_{}".format(sample_name))
            ...
            print("result visulisation saved in link {}".format(
                html_path.replace("/project/3dlg-hcvc/mirrors/www", "http://aspis.cmpt.sfu.ca/projects/mirrors")))
    ...
    def gen_latex_table_with_sample_std(self, method_order_txt, all_info_json, midrule_index):
        ...
        def only_add_std(sublines, metrics_list):
            ...
            for one_line in sublines:
                avg_subline_score.append(
                    [float(one_score.strip()) for one_score in one_line[0].replace('\\', '').split("&")[3:]])
            avg_subline_score = np.array(avg_subline_score)

            std_subline_score = []
            for one_line in sublines:
                std_subline_score.append(
                    [float(one_score.strip()) for one_score in one_line[1].replace('\\', '').split("&")[3:]])
                ...
                for col_index in range(col_num):
                    ...
                    for one_pos in pos_list:
                        ...
                        if one_best_line[one_pos + 6] != "(" and one_best_line[one_pos + 5] != ")":
                            one_best_line = nth_replace(one_best_line, ori_str_scores[col_index],
                                                        "{} ({})".format(ori_str_scores[col_index],
                                                                         std_ori_str_scores[col_index]), occur_index)
            ...
            for one_line in sublines:
                ...
                for col_index in range(col_num):
                    if abs(float(ori_str_scores[col_index]) - best_score[col_index]) < 1e-5:
                        one_best_line = one_best_line.replace(ori_str_scores[col_index],
                                                              "\\best{" + ori_str_scores[col_index] + "}")
                best_lines.append(one_best_line)
        ...
        for exp_index, method_tag in enumerate(method_order_list):
            if len(methodTag_info[method_tag]) >= 3:
                std_one_latex_lines = [item for item in std_methodTag_info[method_tag][
                    -3].items()]  # TODO -1 is nyu normal 0 is m3d normal
                one_latex_lines = [item for item in
                                   methodTag_info[method_tag][-3].items()]  # using expriment 0 's result 
            else:
                std_one_latex_lines = [item for item in std_methodTag_info[method_tag][
                    -1].items()]  # TODO -1 is nyu normal 0 is m3d normal
                one_latex_lines = [item for item in
                                   methodTag_info[method_tag][-1].items()]  # using expriment 0 's result 
            ...
            if ((exp_index + 1) in midrule_index) or (exp_index == len(method_order_list) - 1):
                ...
                if "*" not in method_tag:
                    main_table_lines_sub = identify_best_for_subLines(main_table_lines_sub,
                                                                      main_table_lines_metrics_list)
                    sup_table_part1_lines_sub = identify_best_for_subLines(sup_table_part1_lines_sub,
                                                                           sup_table_part1_lines_metrics_list)
                    sup_table_part2_lines_sub = identify_best_for_subLines(sup_table_part2_lines_sub,
                                                                           sup_table_part2_lines_metrics_list)

                else:
                    main_table_lines_sub = only_add_std(main_table_lines_sub, main_table_lines_metrics_list)
                    sup_table_part1_lines_sub = only_add_std(sup_table_part1_lines_sub,
                                                             sup_table_part1_lines_metrics_list)
                    sup_table_part2_lines_sub = only_add_std(sup_table_part2_lines_sub,
                                                             sup_table_part2_lines_metrics_list)
        ...
        sup_table_part2_lines.append(
            "\\bottomrule\end{tabular}}\caption{ Additional quantitative metrics for " + caption + " ; avg score (sample SE)}\end{table*}")
    ...
    def gen_latex_table_whole(self, method_order_txt, all_info_json, midrule_index):
        ...
        def identify_best_for_subLines(sublines, metrics_list):
            ...
            subline_score = []
            for one_line in sublines:
                subline_score.append(
                    [float(one_score.strip()) for one_score in one_line.replace('\\', '').split("&")[3:]])
        ...
        for exp_index, method_tag in enumerate(method_order_list):
            ...
            one_latex_lines = [item for item in
                               methodTag_info[method_tag][-1].items()]  # TODO -1 is nyu normal 0 is m3d normal
        ...
        sup_table_part1_lines.append("\\bottomrule\end{tabular}}")
        sup_table_part2_lines.append(
            "\\bottomrule\end{tabular}}\caption{ Additional quantitative metrics for " + caption + "}\end{table*}")
    ...
    def get_std_score(self, args):
        ...
        for item_index, item in enumerate(self.method_predFolder.items()):
            ...
            if "m3d" in self.dataset_main_folder:
                ...
                dataset_name = "nyu"
            mirror3d_eval = Mirror3dEval(refined_depth, logger=None, Input_tag=Input_tag, method_tag=method_tag,
            ...
            for one_pred_name in tqdm(test_sample_name_list):
                ...
                mirror3d_eval.compute_and_update_mirror3D_metrics(pred_depth / depth_shift, depth_shift,
                                                                  color_image_path)
    ...
    def generate_paper_html(self, vis_saved_main_folder="", sample_num_per_page=50, template_path=""):
        ...
        if self.is_matterport3d:
            method_folder_list = ["GT", "mesh-D", "mesh-D+Mirror3DNet", "saic-ref", "saic-raw", "saic+Mirror3DNet",
                                  "BTS-ref", "BTS-raw", "BTS+Mirror3DNet", "VNL-ref", "VNL-raw", "VNL+Mirror3DNet"]
        else:
            method_folder_list = ["GT", "sensor-D", "sensor-D+Mirror3DNet-m3d", "saic-ref", "saic-raw",
                                  "saic+Mirror3DNet-m3d", "BTS-ref", "BTS-raw", "BTS+Mirror3DNet-m3d", "VNL-ref",
                                  "VNL-raw", "VNL+Mirror3DNet-m3d"]
        # TODO if want to sort by RMSE can sort the self.color_img_list by RMSE score here 
        color_img_subset_list = [self.color_img_list[x:x + sample_num_per_page] for x in
                                 range(0, len(self.color_img_list), sample_num_per_page)]
        for html_index, one_colorSubset in enumerate(color_img_subset_list):
            ...
            for one_color_img_path in one_colorSubset:
                ...
                for method_index, one_method_name in enumerate(method_folder_list):
                    ...
                    if row_num == 1 and is_first_col:
                        ...
                        raw_depth_map = os.path.relpath(
                            os.path.join(os.path.join(vis_saved_main_folder, method_folder_list[1]),
                                         "colored_pred_depth", one_depth_sample_name), self.output_folder)
    ...
    def gen_latex_table_with_run_SE(self, method_order_txt, all_info_json, midrule_index):
        ...
        def only_add_sample_std(sublines, metrics_list):
            ...
            for one_line in sublines:
                ...
                for col_index in range(col_num):
                    ...
                    for one_pos in pos_list:
                        occur_index += 1
                        if one_best_line[one_pos + one_pos_offset + 6] != "(" and one_best_line[
                            one_pos + one_pos_offset + 5] != ")":
                            runs_score = np.array(
                                [float(first_str_scores[col_index]), float(second_str_scores[col_index]),
                                 float(third_str_scores[col_index])])
                            avg_score = "{:.3f}".format(np.mean(runs_score))
                            runs_std = "{:.3f}".format(np.std(runs_score) / np.sqrt(3))
                            one_best_line = nth_replace(one_best_line, first_str_scores[col_index],
                                                        "{} ({})".format(avg_score, runs_std), occur_index)
                            one_pos_offset += 8
        ...
        def identify_best_for_subLines(sublines, metrics_list):
            ...
            for one_line in lines_with_runs_std:
                avg_subline_score.append(
                    [float(one_score.strip().split('(')[0]) for one_score in one_line.replace('\\', '').split("&")[3:]])
            avg_subline_score = np.array(avg_subline_score)
            ...
            for one_line in lines_with_runs_std:
                one_best_line = one_line
                one_avg_score_list = [one_score.strip().split('(')[0] for one_score in
                                      one_line.replace('\\', '').split("&")[3:]]
                for col_index in range(col_num):
                    if abs(float(one_avg_score_list[col_index]) - best_score[col_index]) < 1e-5:
                        one_best_line = one_best_line.replace(one_avg_score_list[col_index],
                                                              "\\best{" + one_avg_score_list[col_index] + "}")
                best_lines.append(one_best_line)

            return best_lines

        method_order_list = [line.strip().replace('\\\\', '\\') for line in read_txt(method_order_txt)]
        method_tag_info = read_json(all_info_json)
        caption = all_info_json.split("/")[-1].replace("_", "\_")
        ...
        for exp_index, method_tag in enumerate(method_order_list):
            if len(method_tag_info[method_tag]) >= 3:
                first_run_latex_lines = [item for item in method_tag_info[method_tag][-3].items()]
                second_run_latex_lines = [item for item in method_tag_info[method_tag][-2].items()]
                third_run_latex_lines = [item for item in method_tag_info[method_tag][-1].items()]
            else:
                first_run_latex_lines = [item for item in method_tag_info[method_tag][-1].items()]
                second_run_latex_lines = [item for item in method_tag_info[method_tag][-1].items()]
                third_run_latex_lines = [item for item in method_tag_info[method_tag][-1].items()]

            main_table_lines_sub.append([first_run_latex_lines[0][1] + "\\", second_run_latex_lines[0][1] + "\\",
                                         third_run_latex_lines[0][1] + "\\"])
            main_table_lines_metrics_list.append(first_run_latex_lines[0][0])

            sup_table_part1_lines_sub.append([first_run_latex_lines[1][1] + "\\", second_run_latex_lines[1][1] + "\\",
                                              third_run_latex_lines[1][1] + "\\"])
            sup_table_part1_lines_metrics_list.append(first_run_latex_lines[1][0])

            sup_table_part2_lines_sub.append([first_run_latex_lines[2][1] + "\\", second_run_latex_lines[2][1] + "\\",
                                              third_run_latex_lines[2][1] + "\\"])
            ...
            if ((exp_index + 1) in midrule_index) or (exp_index == len(method_order_list) - 1):
                ...
                if "*" not in method_tag:
                    ...
                    sup_table_part1_lines_sub = only_add_sample_std(sup_table_part1_lines_sub,
                                                                    sup_table_part1_lines_metrics_list)
                    sup_table_part2_lines_sub = only_add_sample_std(sup_table_part2_lines_sub,
                                                                    sup_table_part2_lines_metrics_list)
        ...
        main_table_lines.append(
            "\\bottomrule\end{tabular}} \caption{" + caption + " ; avg score (3 run's SE)}\end{table}")
        sup_table_part1_lines.append("\\bottomrule\end{tabular}}")
        sup_table_part2_lines.append(
            "\\bottomrule\end{tabular}}\caption{ Additional quantitative metrics for " + caption + " ; avg score (3 run's SE)}\end{table*}")
    ...
    def gen_latex_table_complete(self, method_order_txt, all_info_json, midrule_index, complete_json_path):
        ...
        def only_add_complete(sublines, metrics_list):
            ...
            for one_line in sublines:
                ori_str_scores = [one_score.strip() for one_score in one_line[0].replace('\\', '').split("&")[3:]]
                complete_ori_str_scores = [one_score.strip() for one_score in
                                           one_line[1].replace('\\', '').split("&")[3:]]
                one_best_line = one_line[0]
                # add complete score 
                for col_index in range(col_num):
                    pos_list = [m.start() for m in re.finditer(ori_str_scores[col_index], one_best_line)]
                    ...
                    for one_pos in pos_list:
                        ...
                        if one_best_line[one_pos + 6] != "(" and one_best_line[one_pos + 5] != ")":
                            one_best_line = nth_replace(one_best_line, ori_str_scores[col_index],
                                                        "{} ({})".format(ori_str_scores[col_index],
                                                                         complete_ori_str_scores[col_index]),
                                                        occur_index)
        ...
        for exp_index, method_tag in enumerate(method_order_list):
            ...
            if ((exp_index + 1) in midrule_index) or (exp_index == len(method_order_list) - 1):
                ...
                if "*" not in method_tag:
                    ...
                    sup_table_part1_lines_sub = only_add_complete(sup_table_part1_lines_sub,
                                                                  sup_table_part1_lines_metrics_list)
                    sup_table_part2_lines_sub = only_add_complete(sup_table_part2_lines_sub,
                                                                  sup_table_part2_lines_metrics_list)
    ...
    def gen_latex_table_without_run_SE(self, method_order_txt, all_info_json, midrule_index):
        ...
        def identify_best_for_subLines(sublines, metrics_list):
            ...
            for one_line in lines_with_runs_std:
                try:
                    avg_subline_score.append([float(one_score.strip().split('(')[0]) for one_score in
                                              one_line.replace('\\', '').split("&")[3:]])
                except:
                    print()
            avg_subline_score = np.array(avg_subline_score)
    ...
    def gen_latex_table_from_tagFolder(self, method_order_txt, all_info_txt, midrule_index, compare_with_raw,
                                       dataset_tag="NYUv2"):
        import re
        ...
        def update_latex_tables(tables, table_scores, info_path_list, method_tag, compare_with_raw, block=[0, 1]):
            ...
            single_std_score = np.std(score_list[single_sample_num:2 * single_sample_num], axis=0) / np.sqrt(
                single_sample_num)

            Input_tag, Train_tag, latex_method_tag = method_tag.split(",")
            tables["main_without_SE"].append(
                "{:>5} & {:>10} & {:>45} & {:.3f} & {:.3f} & {:.3f} & {:.3f} & {:.3f} & {:.3f} \\\\".format(
                    Input_tag, Train_tag, latex_method_tag, avg_score[0], avg_score[9], avg_score[18], \
                    avg_score[3], avg_score[12], avg_score[21]))
            tables["main_with_SE"].append(
                "{:>5} & {:>10} & {:>45} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} \\\\".format(
                    Input_tag, Train_tag, latex_method_tag, avg_score[0], std_score[0], avg_score[9], std_score[9],
                    avg_score[18], std_score[18], \
                    avg_score[3], std_score[3], avg_score[12], std_score[12], avg_score[21], std_score[21]))
            tables["sup_multi_with_SE"][0].append(
                "{:>5} & {:>10} & {:>45} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f}\\\\".format(
            ...
            tables["sup_multi_with_SE"][1].append(
                "{:>5} & {:>10} & {:>45} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f}\\\\".format(
                    Input_tag, Train_tag, latex_method_tag,
            ...
            tables["sup_single_with_SE"][0].append(
                "{:>5} & {:>10} & {:>45} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f}\\\\".format(
                    Input_tag, Train_tag, latex_method_tag,
                    single_avg_score[0], single_std_score[0], single_avg_score[9], single_std_score[9],
                    single_avg_score[18], single_std_score[18],
                    single_avg_score[1], single_std_score[1], single_avg_score[10], single_std_score[10],
                    single_avg_score[19], single_std_score[19],
                    single_avg_score[2], single_std_score[2], single_avg_score[11], single_std_score[11],
                    single_avg_score[20], single_std_score[20],
                    single_avg_score[3], single_std_score[3], single_avg_score[12], single_std_score[12],
                    single_avg_score[21], single_std_score[21]
                ))
            tables["sup_single_with_SE"][1].append(
                "{:>5} & {:>10} & {:>45} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f} & {:.3f} $\pm$ {:.3f}\\\\".format(
                    Input_tag, Train_tag, latex_method_tag,
                    single_avg_score[4], single_std_score[4], single_avg_score[13], single_std_score[13],
                    single_avg_score[22], single_std_score[22],
                    single_avg_score[5], single_std_score[5], single_avg_score[14], single_std_score[14],
                    single_avg_score[23], single_std_score[23],
                    single_avg_score[6], single_std_score[6], single_avg_score[15], single_std_score[15],
                    single_avg_score[24], single_std_score[24],
                    single_avg_score[7], single_std_score[7], single_avg_score[16], single_std_score[16],
                    single_avg_score[25], single_std_score[25],
                    single_avg_score[8], single_std_score[8], single_avg_score[17], single_std_score[17],
                    single_avg_score[26], single_std_score[26]
                ))
            # if 0 not in block:
            tables["main_without_SE"][-1] = update_line_with_best(tables["main_without_SE"][-1],
                                                                  np.array(table_scores["main"])[block],
                                                                  is_up=[False, True])
            tables["main_with_SE"][-1] = update_line_with_best(tables["main_with_SE"][-1],
                                                               np.array(table_scores["main"])[block],
                                                               is_up=[False, True])
            tables["sup_multi_with_SE"][0][-1] = update_line_with_best(tables["sup_multi_with_SE"][0][-1],
                                                                       np.array(table_scores["sup_multi_run"][0])[
                                                                           block], is_up=[False, False, False, True])
            tables["sup_multi_with_SE"][1][-1] = update_line_with_best(tables["sup_multi_with_SE"][1][-1],
                                                                       np.array(table_scores["sup_multi_run"][1])[
                                                                           block], is_up=[True, True, True, True, True])
            tables["sup_single_with_SE"][0][-1] = update_line_with_best(tables["sup_single_with_SE"][0][-1],
                                                                        np.array(table_scores["sup_single_run"][0])[
                                                                            block], is_up=[False, False, False, True])
            tables["sup_single_with_SE"][1][-1] = update_line_with_best(tables["sup_single_with_SE"][1][-1],
                                                                        np.array(table_scores["sup_single_run"][1])[
                                                                            block],
                                                                        is_up=[True, True, True, True, True])
        ...
        method_order_list = [line.strip().replace('\\\\', '\\') for line in read_txt(method_order_txt)]
        method_tag_info_file = dict()
        for one_line in read_txt(all_info_txt):
            methodTag, infoFolder = one_line.split(":")
            methodTag = methodTag.strip().replace('\\\\', '\\')
            if methodTag in method_tag_info_file:
                method_tag_info_file[methodTag].append(
                    os.path.join(infoFolder, "minFilter_True_full_False_score_per_sample.json"))
            else:
                method_tag_info_file[methodTag] = [
                    os.path.join(infoFolder, "minFilter_True_full_False_score_per_sample.json")]
        ...
        for exp_index, method_tag in enumerate(method_order_list):
            table_scores = update_table_scores(table_scores, method_tag_info_file[method_tag], compare_with_raw)
        ...
        for exp_index, method_tag in enumerate(method_order_list):
            ...
            tables = update_latex_tables(tables, table_scores, method_tag_info_file[method_tag], method_tag,
                                         compare_with_raw, block)
        ...
        tables["main_without_SE"].append(
            "\\bottomrule\end{tabular}} \caption{Result on " + "{}-{}".format(dataset_tag, input_tag) + "}\end{table}")
        tables["main_with_SE"].append("\\bottomrule\end{tabular}} \caption{Result on " + "{}-{}".format(dataset_tag,
                                                                                                        input_tag) + " with SE}\end{table}")
        tables["sup_single_with_SE"][1].append(
            "\\bottomrule\end{tabular}}\caption{ Additional quantitative metrics for " + "{}-{}".format(dataset_tag,
                                                                                                        input_tag) + " across a single run}\end{table*}")
        tables["sup_multi_with_SE"][1].append(
            "\\bottomrule\end{tabular}}\caption{ Additional quantitative metrics for " + "{}-{}".format(dataset_tag,
                                                                                                        input_tag) + " across all runs}\end{table*}")
    ...
    def gen_latex_table_from_tagFolder_bold_separate(self, method_order_txt, all_info_txt, midrule_index,
                                                     compare_with_raw, dataset_tag="NYUv2"):
        import re
        ...
        def update_latex_tables(tables, table_scores, info_path_list, method_tag, compare_with_raw, block=[0, 1]):
            ...
            # if 0 not in block:
            tables["main_without_SE"][-1] = update_line_with_best_metrics(tables["main_without_SE"][-1],
                                                                          np.array(table_scores["metrics"]["main"])[
                                                                              block], is_up=[False, True])
            tables["main_with_SE"][-1] = update_line_with_best_metrics(tables["main_with_SE"][-1],
                                                                       np.array(table_scores["metrics"]["main"])[block],
                                                                       is_up=[False, True])
            tables["sup_multi_with_SE"][0][-1] = update_line_with_best_metrics(tables["sup_multi_with_SE"][0][-1],
                                                                               np.array(table_scores["metrics"][
                                                                                            "sup_multi_run"][0])[block],
                                                                               is_up=[False, False, False, True])
            tables["sup_multi_with_SE"][1][-1] = update_line_with_best_metrics(tables["sup_multi_with_SE"][1][-1],
                                                                               np.array(table_scores["metrics"][
                                                                                            "sup_multi_run"][1])[block],
                                                                               is_up=[True, True, True, True, True])
            tables["sup_single_with_SE"][0][-1] = update_line_with_best_metrics(tables["sup_single_with_SE"][0][-1],
                                                                                np.array(table_scores["metrics"][
                                                                                             "sup_single_run"][0])[
                                                                                    block],
                                                                                is_up=[False, False, False, True])
            tables["sup_single_with_SE"][1][-1] = update_line_with_best_metrics(tables["sup_single_with_SE"][1][-1],
                                                                                np.array(table_scores["metrics"][
                                                                                             "sup_single_run"][1])[
                                                                                    block],
                                                                                is_up=[True, True, True, True, True])

            tables["main_with_SE"][-1] = update_line_with_best_std(tables["main_with_SE"][-1],
                                                                   np.array(table_scores["se"]["main"])[block],
                                                                   is_up=[False, True])
            tables["sup_multi_with_SE"][0][-1] = update_line_with_best_std(tables["sup_multi_with_SE"][0][-1], np.array(
                table_scores["se"]["sup_multi_run"][0])[block], is_up=[False, False, False, True])
            tables["sup_multi_with_SE"][1][-1] = update_line_with_best_std(tables["sup_multi_with_SE"][1][-1], np.array(
                table_scores["se"]["sup_multi_run"][1])[block], is_up=[True, True, True, True, True])
            tables["sup_single_with_SE"][0][-1] = update_line_with_best_std(tables["sup_single_with_SE"][0][-1],
                                                                            np.array(
                                                                                table_scores["se"]["sup_single_run"][
                                                                                    0])[block],
                                                                            is_up=[False, False, False, True])
            tables["sup_single_with_SE"][1][-1] = update_line_with_best_std(tables["sup_single_with_SE"][1][-1],
                                                                            np.array(
                                                                                table_scores["se"]["sup_single_run"][
                                                                                    1])[block],
                                                                            is_up=[True, True, True, True, True])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--test_json',
        default="/project/3dlg-hcvc/mirrors/www/Mirror3D_final/nyu/with_mirror/precise/network_input_json"
                "/test_10_normal_mirror.json")
    parser.add_argument(
    ...
    parser.add_argument(
        '--pred_w', default=640, type=int, help="width of the visualization window")
    parser.add_argument(
        '--pred_h', default=512, type=int, help="width of the visualization window")
    parser.add_argument(
        '--window_w', default=800, type=int, help="width of the visualization window")
    parser.add_argument(
        '--window_h', default=800, type=int, help="height of the visualization window")
    parser.add_argument(
        '--sample_num_per_page', default=100, type=int, help="height of the visualization window")
    parser.add_argument(
    ...
    parser.add_argument("--midrule_index", nargs="+", type=int, default=[2, 5],
                        help="add /midrule in after these liens; index start from 1")
    parser.add_argument(
    ...
    parser.add_argument('--add_rmse', help='add rmse to result visualization or not', action='store_true')
    args = parser.parse_args()

    vis_tool = DatasetVisualization(pred_w=args.pred_w, pred_h=args.pred_h,
                                    dataset_main_folder=args.dataset_main_folder, process_index=args.process_index,
    ...
    if args.stage == "1":
        ...
        vis_tool.generate_html(vis_saved_main_folder=args.vis_saved_folder,
                               sample_num_per_page=args.sample_num_per_page, template_path=args.template_path)
    elif args.stage == "3":
        ...
        vis_tool.generate_paper_html(vis_saved_main_folder=args.vis_saved_folder,
                                     sample_num_per_page=args.sample_num_per_page,
                                     template_path="visualization/paper_vis_template.html")
    elif args.stage == "7":
        ...
        complete_json_path = "/project/3dlg-hcvc/mirrors/www/notes/0512_nyu_complete" \
                             "/raw_nyu_minFilter_True_full_True_result.json "
        vis_tool.gen_latex_table_complete(args.method_order_txt, args.all_info_json, args.midrule_index,
                                          complete_json_path)
    elif args.stage == "10":
        vis_tool.gen_latex_table_without_run_SE(args.method_order_txt, args.all_info_json, args.midrule_index)
    elif args.stage == "11":
        vis_tool.gen_latex_table_from_tagFolder(args.method_order_txt, args.all_info_json, args.midrule_index,
                                                args.compare_with_raw, args.dataset_tag)
    elif args.stage == "12":
        vis_tool.gen_latex_table_from_tagFolder_bold_separate(args.method_order_txt, args.all_info_json,
                                                              args.midrule_index, args.compare_with_raw,
                                                              args.dataset_tag)
