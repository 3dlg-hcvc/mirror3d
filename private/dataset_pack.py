...
def generate_symlinks_txt_mp3d():
    ...
    save_path = "/local-scratch/jiaqit/exp/Mirror3D/dataset/metadata/mp3d_symlink.txt"
    for one_path in all_color_list:
        id = one_path.split("/")[-1].split(".")[0]
        scene_id = one_path.split("/")[-3]
        if id in all_id:
            color_to_link = "mirror_color_images/{}/{}".format(scene_id,one_path.split("/")[-1])
            sensorD_to_link = "raw_sensorD/{}/{}".format(scene_id,rreplace(one_path.replace("color","depth").replace("jpg","png"),"i","d").split("/")[-1])
            meshD_to_link = "raw_meshD/{}/{}".format(scene_id,rreplace(one_path.replace("color","depth").replace("jpg","png"),"i","d").split("/")[-1])
