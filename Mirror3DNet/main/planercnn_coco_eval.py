...
class PlanercnnCOCOeval(COCOeval):
    ...
    def evaluate_normal(self, cfg):
        ...
        def eval_normal():
            ...
            # for iou in self.params.iouThrs:
            #     if len(iou_predNum_noneNum[iou][0]):
            #         IOU_list.append([np.array(iou_l2lossList[iou])[:,0].mean(), np.array(iou_l2lossList[iou])[:,1].mean(), np.array(iou_l2lossList[iou])[:,2].mean()])


            # if cfg.ANCHOR_CLS:
            #     print("| {:45} | IOU 0.5:0.95 | L2_loss {:5f} |".format("mirror_pred_anchor & mirror_GT_normal" ,np.array(IOU_list)[:,0].mean()))
            # if cfg.ANCHOR_REG and cfg.ANCHOR_CLS:
            #     print("| {:45} | IOU 0.5:0.95 | L2_loss {:5f} |".format("mirror_pred_normal & mirror_GT_normal" ,np.array(IOU_list)[:,1].mean()))
            # if cfg.ANCHOR_REG:
            #     print("| {:45} | IOU 0.5:0.95 | L2_loss {:5f} |".format("mirror_pred_res & mirror_GT_res" ,np.array(IOU_list)[:,2].mean()))

            # #################################### get cos similarity ####################################
            # IOU_list = []
            # for iou in self.params.iouThrs:
            #     if len(iou_predNum_noneNum[iou][0]):
            #         IOU_list.append([np.array(iou_CSlist[iou])[:,0].mean(), np.array(iou_CSlist[iou])[:,1].mean(), np.array(iou_CSlist[iou])[:,2].mean()])

            # if cfg.ANCHOR_CLS:
            #     print("| {:45} | IOU 0.5:0.95 | SSIM {:5f} |".format("mirror_pred_anchor & mirror_GT_normal" ,np.array(IOU_list)[:,0].mean()))
            # if cfg.ANCHOR_REG and cfg.ANCHOR_CLS:
            #     print("| {:45} | IOU 0.5:0.95 | SSIM {:5f} |".format("mirror_pred_normal & mirror_GT_normal" ,np.array(IOU_list)[:,1].mean()))
            # if cfg.ANCHOR_REG:
            #     print("| {:45} | IOU 0.5:0.95 | SSIM {:5f} |".format("mirror_pred_res & mirror_GT_res" ,np.array(IOU_list)[:,2].mean()))

            # #################################### get angle loss ####################################
            # IOU_list = []
            # for iou in self.params.iouThrs:
            #     if len(iou_predNum_noneNum[iou][0]):
            #         IOU_list.append([np.array(iou_angle_list[iou])[:,0].mean(), np.array(iou_angle_list[iou])[:,1].mean(), np.array(iou_angle_list[iou])[:,2].mean()])

            # if cfg.ANCHOR_CLS:
            #     print("| {:45} | IOU 0.5:0.95 | angle_diff {:5f} |".format("mirror_pred_anchor & mirror_GT_normal" ,np.array(IOU_list)[:,0].mean()))
            # if cfg.ANCHOR_REG and cfg.ANCHOR_CLS:
            #     print("| {:45} | IOU 0.5:0.95 | angle_diff {:5f} |".format("mirror_pred_normal & mirror_GT_normal" ,np.array(IOU_list)[:,1].mean()))
            # if cfg.ANCHOR_REG:
            #     print("| {:45} | IOU 0.5:0.95 | angle_diff {:5f} |".format("mirror_pred_res & mirror_GT_res" ,np.array(IOU_list)[:,2].mean()))

        eval_normal()

        
        ################################### get ANCHOR_REG and anchor_cls performance plot ####################################
        # if cfg.EVAL_SAVE_NORMAL_VIS:
        #     colors = ["#"+''.join([random.choice('0123456789ABCDEF') for j in range(6)]) for i in range(len(iou_l2lossList))]
        #     distances = []

        #     #################################### plot scatter points ####################################
        #     plt.cla()
        #     for iou_idx, one_iou_errorList in enumerate(iou_l2lossList.items()):
        #         iou = one_iou_errorList[0]
        #         x = []
        #         y = []
        #         for errors in one_iou_errorList[1]:
        #             y.append(errors[-2]) # improvement 
        #             x.append(errors[-1]) # distance
        #             distances.append(errors[-1])
        #         plt.scatter(x, y, c=colors[iou_idx], alpha=0.4, lw=0, label="{:.2f}".format(iou))

        #     plt.xlabel("distance")
        #     plt.ylabel("improvement")
        #     plt.legend()
        #     scatter_img_save_name = time_info + cfg.MODEL.WEIGHTS.split("/")[-2] + "_scatter.jpg"
        #     plt.savefig(os.path.join(img_save_folder, scatter_img_save_name))
        #     print("plot saved to : ", os.path.join(img_save_folder, scatter_img_save_name))

        #     #################################### plot line ####################################

        #     def get_distance_range(distance_xx, distance):
        #         for dis_index, one_distance_x in enumerate(distance_xx):
        #             if distance > distance_xx[dis_index] and distance < distance_xx[dis_index+1]:
        #                 return distance_xx[dis_index]

        #     distance_xx = np.arange(0, max(distances)+0.05, 0.05)
        #     distance_meanImprove = dict()
        #     for one_distance_x in distance_xx:
        #         distance_meanImprove[one_distance_x] = []

        #     # init
        #     distance_IOU_imrpove = dict()
        #     for one_distance_x in distance_xx:
        #         distance_IOU_imrpove[one_distance_x] = dict()
        #         for iou in self.params.iouThrs:
        #             distance_IOU_imrpove[one_distance_x][iou] = []

        #     # convert iou_l2lossList to distance_IOU_imrpove
        #     for one_iou_errorList in iou_l2lossList.items():
        #         iou = one_iou_errorList[0]
        #         for errorList in one_iou_errorList[1]:
        #             improve = errorList[-2]
        #             distance_x = get_distance_range(distance_xx, errorList[-1])
        #             distance_IOU_imrpove[distance_x][iou].append(improve)
            
        #     # get xx, yy, count information from distance_IOU_imrpove to plot
        #     xx = []
        #     yy = []
        #     sample_count_list = []
        #     for distance_info in distance_IOU_imrpove.items():
        #         distance_x = distance_info[0]
        #         IOU_improve = distance_info[1]
        #         all_IOU_improve_list = []
        #         sample_count = 0
        #         for one_iou_improve in IOU_improve.items():
        #             if len(one_iou_improve[1]) > 0:
        #                 all_IOU_improve_list.append(np.mean(one_iou_improve[1]))
        #                 sample_count += len(one_iou_improve[1])
        #         current_distance_mean_improve = np.mean(all_IOU_improve_list)
        #         if len(all_IOU_improve_list) > 0:
        #             xx.append(distance_x)
        #             yy.append(current_distance_mean_improve)
        #             sample_count_list.append(sample_count)

        #     # plot line chart
        #     plt.cla()
        #     plt.plot(xx, yy,'o-', label="number of samples" ) # plot for different IOU threshold
        #     for x, y ,count in zip(xx,yy, sample_count_list):
        #         plt.text(x, y, str(count) )
        #     plt.xlabel("distance")
        #     plt.ylabel("improvement")
        #     plt.legend()
        #     lineChart_img_save_name = time_info + cfg.MODEL.WEIGHTS.split("/")[-2] + "_dis_VS_ip.jpg"
        #     plt.savefig(os.path.join(img_save_folder, lineChart_img_save_name))
        #     print("plot saved to : ", os.path.join(img_save_folder, lineChart_img_save_name))
