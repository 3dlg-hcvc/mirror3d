...
from planrcnn_detectron2_lib.structures import BitMasks, Boxes, BoxMode, Keypoints, PolygonMasks, RotatedBoxes
...
def _create_text_labels_chris(pred_anchor_classes, anchor_scores, scores):
    """
    Args:
        classes (list[int] or None):
        anchor_scores (list[float] or None):
    ...
    if anchor_scores is not None:
        labels = ["{} {:.0f}% {:.0f}%".format(l, a_s * 100, s * 100) for l,a_s, s in zip(pred_anchor_classes, anchor_scores, scores)]
    return labels
...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        pred_anchor_classes = predictions.pred_anchor_classes if predictions.has("pred_anchor_classes") else None
        anchor_scores = predictions.anchor_scores if predictions.has("anchor_scores") else None
        labels = _create_text_labels_chris(pred_anchor_classes, anchor_scores, scores)
        keypoints = predictions.pred_keypoints if predictions.has("pred_keypoints") else None
        chris_colors = [random_color(rgb=True, maximum=1) for _ in range(len(boxes))]
        ...
        self.overlay_instances(
            ...
            labels=labels,
            keypoints=keypoints,
            assigned_colors=chris_colors,
            alpha=alpha,
        )
        return self.output, chris_colors
        ...
        *,
        ...
        Args:
            ...
            masks (masks-like object): Supported types are:
                ...
                * :class:`planrcnn_detectron2_lib.structures.PolygonMasks`,
                  :class:`planrcnn_detectron2_lib.structures.BitMasks`.
